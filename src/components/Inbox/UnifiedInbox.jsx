...
function Avatar({ name, platform, size = 10 }) {
  const p = PLATFORM_STYLES[platform] || PLATFORM_STYLES.facebook;
  return (
    <div
      className={`w-${size} h-${size} rounded-full flex items-center justify-center text-white font-bold text-sm flex-shrink-0 ${p.bg}`}
    >
      {name?.[0]?.toUpperCase() || '?'}
    </div>
  );
...
function MessageCard({ comment, onReply, onMarkRead }) {
...
export default function UnifiedInbox() {
  ...
  const [filters, setFilters] = useState({ comment_type: '', platform: '', is_read: '' });
  ...
  // Filter pills
  const FILTER_PILLS = [
    { label: 'All', key: 'comment_type', value: '' },
    { label: '💬 Comments', key: 'comment_type', value: 'comment' },
    { label: '✉️ DMs', key: 'comment_type', value: 'dm' },
    { label: '📘 Facebook', key: 'platform', value: 'facebook' },
    { label: '📸 Instagram', key: 'platform', value: 'instagram' },
    { label: '🔵 Unread', key: 'is_read', value: '0' },
    { label: '✓ Read', key: 'is_read', value: '1' },
  ];
